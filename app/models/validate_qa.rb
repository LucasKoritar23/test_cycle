class ValidateQa
  ...
  def validate_edit_qa(qa_params)
    ...
    if name.nil?
      error = { error: "Nome não pode ser nulo" }
    elsif name.class != String
      error = { error: "Nome deve ser string" }
    elsif name.empty?
      error = { error: "Nome não pode ser em branco" }
    elsif name.length < 3 or name.length > 200
      error = { error: "Nome deve conter entre 3 e 200 caracteres" }
    end
    error
  end

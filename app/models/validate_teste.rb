class ValidateTeste
  ...
  def unique_value(error)
    current_message = error.messages[:suite][0] if error.messages.keys.include?(:suite)
    current_message = error.messages[:nome][0] if error.messages.keys.include?(:nome)
    new_message = "Não existe uma suite com o id informado" if current_message == "must exist"
    new_message.nil? ? message_send = current_message : message_send = new_message
    { error: message_send }
  end
end